...
def test_failure_resume_without_mechanistic_support_verifies_cleanly(tmp_path: Path) -> None:
    config_path = tmp_path / "config.yaml"
    base = Path("examples/synthetic").resolve()
    raw = yaml.safe_load(Path("examples/synthetic/config.yaml").read_text())
    raw["sequence"]["transcript_fasta"] = str(base / raw["sequence"]["transcript_fasta"])
    ...
    raw["pathway"]["network_file"] = str(base / raw["pathway"]["network_file"])
    raw["pathway"]["regulon_file"] = str(base / raw["pathway"]["regulon_file"])
    raw.setdefault("residual_attribution", {})["use_mechanistic_pathway_support"] = False
    raw["residual_attribution"]["pathway_support_source"] = "mechanistic_network"
    raw.setdefault("execution", {})["inject_failure_stage"] = "expression_analysis"
    config_path.write_text(yaml.safe_dump(raw, sort_keys=False))
    ...
    try:
        ...
        raise AssertionError("expected controlled expression failure")

    rows = run_staged_analysis(config_path=config_path, output_dir=out, resume=True)

    assert not any(row["stage"] == "mechanistic_network" for row in rows)
